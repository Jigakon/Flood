...
//	}
	...
	{
		ivec4 const Pack(round(clamp(v,-1.0f, 1.0f) * vec4(511.0f, 511.0f, 511.0f, 1.0f)));
	...
	GLM_FUNC_QUALIFIER vec4 unpackSnorm3x10_1x2(uint32 v)
	{
		...
		return clamp(Result * vec4(1.0f / 511.0f, 1.0f / 511.0f, 1.0f / 511.0f, 1.0f), -1.0f, 1.0f);
	}

	GLM_FUNC_QUALIFIER uint32 packUnorm3x10_1x2(vec4 const& v)
	{
		uvec4 const Unpack(round(clamp(v, 0.0f, 1.0f) * vec4(1023.0f, 1023.0f, 1023.0f, 3.0f)));
	...
	{
		vec4 const ScaleFactors(1.0f / 1023.0f, 1.0f / 1023.0f, 1.0f / 1023.0f, 1.0f / 3.0f);
	...
	{
		float const SharedExpMax = (pow(2.0f, 9.0f - 1.0f) / pow(2.0f, 9.0f)) * pow(2.0f, 31.0f - 15.0f);
		vec3 const Color = clamp(v, 0.0f, SharedExpMax);
		float const MaxColor = max(Color.x, max(Color.y, Color.z));

		float const ExpSharedP = max(-15.0f - 1.0f, floor(log2(MaxColor))) + 1.0f + 15.0f;
		float const MaxShared = floor(MaxColor / pow(2.0f, (ExpSharedP - 15.0f - 9.0f)) + 0.5f);
		float const ExpShared = equal(MaxShared, pow(2.0f, 9.0f), epsilon<float>()) ? ExpSharedP + 1.0f : ExpSharedP;

		uvec3 const ColorComp(floor(Color / pow(2.0f, (ExpShared - 15.0f - 9.0f)) + 0.5f));
	...
	GLM_FUNC_QUALIFIER vec3 unpackF3x9_E1x5(uint32 v)
	{
		...
		return vec3(Unpack.data.x, Unpack.data.y, Unpack.data.z) * pow(2.0f, Unpack.data.w - 15.0f - 9.0f);
	}
	...
	{
		float const ScaleFactor(1.0f / 15.0f);
		detail::u4u4 Unpack;
		Unpack.pack = v;
	...
	{
		u32vec3 const Unpack(round(clamp(v, 0.0f, 1.0f) * vec3(31.0f, 63.0f, 31.0f)));
	...
	{
		vec3 const ScaleFactor(1.0f / 31.0f, 1.0f / 63.0f, 1.0f / 31.0f);
	...
	{
		u32vec4 const Unpack(round(clamp(v, 0.0f, 1.0f) * vec4(31.0f, 31.0f, 31.0f, 1.0f)));
	...
	{
		vec4 const ScaleFactor(1.0f / 31.0f, 1.0f / 31.0f, 1.0f / 31.0f, 1.0f);
	...
	{
		u32vec3 const Unpack(round(clamp(v, 0.0f, 1.0f) * vec3(7.0f, 7.0f, 3.0f)));
	...
	{
		vec3 const ScaleFactor(1.0f / 7.0f, 1.0f / 7.0f, 1.0f / 3.0f);
